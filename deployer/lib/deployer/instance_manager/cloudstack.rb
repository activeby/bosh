  ...
  class InstanceManager
    ...
    class Cloudstack < InstanceManager
      ...
      def configure
        ...
        unless key
          raise ConfigError, "Missing properties.cloudstack.private_key"
        end
        @ssh_key = File.expand_path(key)
        unless File.exists?(@ssh_key)
          raise ConfigError, "properties.cloudstack.private_key '#{key}' does not exist"
        end
      ...
      def start
        ...
        unless has_cloudstack_registry?
          raise "cloudstack_registry command not found - " +
        ...
        begin
          ...
          if timeout_time - Time.now.to_f > 0
            ...
            raise "Cannot access cloudstack_registry: #{e.message}"
          end
        end
      ...
      def service_ip
        ip = cloud.cloudstack.servers.get(state.vm_cid).private_ip_address
