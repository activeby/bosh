...
describe Bosh::CloudstackRegistry::Runner do

  def make_runner(config_file)
    Bosh::CloudstackRegistry::Runner.new(config_file)
  end
  ...
  describe "initializing" do
    it "configures CloudStack registry using provided file" do
      config_file = Tempfile.new("config")
      config = { "key" => "value" }
      write_config(config_file.path, config)

      Bosh::CloudstackRegistry.should_receive(:configure).with(config)
    ...
    it "fails if config file not found" do
      expect {
        make_runner("foo")
      }.to raise_error(Bosh::CloudstackRegistry::ConfigError, "Cannot find file `foo'")
    end
    ...
    it "fails when config file has incorrect format" do
      ...
      expect {
        make_runner(config_file.path)
      }.to raise_error(Bosh::CloudstackRegistry::ConfigError, /Incorrect file format/)
    end
    ...
    it "fails when some syscall fails" do
      ...
      expect {
        make_runner(config_file.path)
      }.to raise_error(Bosh::CloudstackRegistry::ConfigError, /baz/)
    end
  end
