...
describe Bosh::CloudstackRegistry::ApiController do

  before(:each) do
    Bosh::CloudstackRegistry.http_user = "admin"
    Bosh::CloudstackRegistry.http_password = "admin"

    @server_manager = mock("server manager")
    Bosh::CloudstackRegistry::ServerManager.stub!(:new).and_return(@server_manager)

    rack_mock = Rack::MockSession.new(Bosh::CloudstackRegistry::ApiController.new)
  ...
  it "returns settings for given CloudStack server (IP check)" do
